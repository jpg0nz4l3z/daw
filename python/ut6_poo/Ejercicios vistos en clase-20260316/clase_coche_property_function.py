...
class Coche:
    ...
    def set_marcha_actual(self, value):
        self._marcha_actual = value
        print(f"Marcha actual actualizada a: {self._marcha_actual}")

    marcha_actual = property(get_marcha_actual, set_marcha_actual)
    ...
    def acelerar(self, velocidad_objetivo):
        ...
        while self.velocidad_actual < velocidad_objetivo:
            ...
            print(f"Acelerando... Velocidad actual: {self.velocidad_actual} km/h, Marcha actual: {self.marcha_actual}")
            time.sleep(1)  # Simula el tiempo que tarda en acelerar

    def actualizar_marcha(self):
        match self.velocidad_actual:
            case v if v <= 30:
                if self.marcha_actual != 1: self.marcha_actual = 1
            case v if v <= 50:
                if self.marcha_actual != 2: self.marcha_actual = 2
            case v if v <= 70:
                if self.marcha_actual != 3: self.marcha_actual = 3
            case v if v <= 100:
                if self.marcha_actual != 4: self.marcha_actual = 4
            case _:
                if self.marcha_actual != 5: self.marcha_actual = 5
    ...
    def mantener_velocidad(self, tiempo):
        ...
        print(f"Manteniendo velocidad de {self.velocidad_actual} km/h durante {tiempo} segundos.")
        time.sleep(tiempo)
    ...
    def desacelerar(self):
        ...
        while self.velocidad_actual > 0:
            self.velocidad_actual -= 5  # Disminuye la velocidad en incrementos de 5 km/h
            self.actualizar_marcha()
            print(f"Desacelerando... Velocidad actual: {self.velocidad_actual} km/h, Marcha actual: {self.marcha_actual}")
...
if __name__ == "__main__":
    ...
    tiempo_mantener = int(input("Introduce el tiempo para mantener la velocidad (segundos): "))
